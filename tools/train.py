...
def get_args_parser():
    ...
    parser.add_argument("--config", default=os.path.join("configs", "exp", "pretrain_day_dj30_dual_vqvae.py"), help="pretrain config file path")
    parser.add_argument(
        '--cfg-options',
        nargs='+',
        action=DictAction,
...
def main(args):
    ...
    # 6. init logger
    logger = Logger(config.log_path, accelerator)
    writer = None
    if config.writer:
        writer = TensorBoardLogger(config.tensorboard_path, accelerator)
    wandb = None
    if config.wandb:
