...
from storm.environment.categorical import Categorical
...
class EnvironmentPatchTrading(gym.Env):
    ...
    def _init_expert_trajectory(self):
        ...
        o, h, l, c, adj = prices[:, 0], prices[:, 1], prices[:, 2], prices[:, 3], prices[:, 4]
        next_adj = adj[1:]
        next_adj = np.append(next_adj, adj[-1])
        ...
        for index, (action_label, price, next_price, timestamp) in enumerate(zip(actions, adj, next_adj, price_timestamps)):
            ...
            self.expert_logprobs[timestamp] = expert_logprob
        ...
        expert_values = self.calculate_expert_values(records["rets"], gamma=self.gamma, use_norm_rewards=self.use_norm_rewards)
        self.expert_values = {timestamp: value for timestamp, value in zip(price_timestamps, expert_values)}
    ...
    def reset(self, **kwargs):
        ...
        self.start_timestamp_index = deepcopy(self.timestamp_index)
        ...
        self.expert_value = self.expert_values[self.timestamp]
        self.expert_logprob = self.expert_logprobs[self.timestamp]
...
if __name__ == '__main__':
    ...
    transition_shape = dict(
        features=dict(shape=(num_envs, num_block_tokens, num_features), type="float32",
    ...
    dataset = dict(
        ...
        exp_path=assemble_project_path(os.path.join("workdir", "patch_trading_testing"))
    )
    ...
    environments = gym.vector.AsyncVectorEnv([
        make_env("Trading-v2", env_params=dict(env=deepcopy(environment),
    ...
    actions = [value for key, value in expert_actions.items()]
