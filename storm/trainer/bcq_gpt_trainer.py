...
class BCQGPTTrainer():
    ...
    def __init__(self,
        ...
        self.train_environments = gym.vector.AsyncVectorEnv([
            make_env("Trading-v1", env_params=dict(env=deepcopy(train_environment),
        ...
        self.valid_environments = gym.vector.AsyncVectorEnv([
            make_env("Trading-v1", env_params=dict(env=deepcopy(valid_environment),
                                                   transition_shape=config.transition_shape, seed=config.seed + i)) for
            i in range(1)
        ])

        self.test_environments = gym.vector.AsyncVectorEnv([
            make_env("Trading-v1", env_params=dict(env=deepcopy(test_environment),
                                                   transition_shape=config.transition_shape, seed=config.seed + i)) for
            i in range(1)
        ])
            ...
            for epoch in range(self.config.update_epochs):
                ...
                for start in range(0, self.config.batch_size, self.config.mini_batch_size):
                    ...
                    with torch.no_grad():
                        ...
                        target_value = reward + done * self.config.discount * target_value.gather(2, next_action.to(torch.int64).unsqueeze(-1)).squeeze(-1)

                    # Get current Q estimate
                    value, scaled_score, score= self.agent(batch)
                    value = value.gather(2, action.to(torch.int64).unsqueeze(-1)).squeeze(-1)

                    # Compute Q loss
                    value_loss = F.smooth_l1_loss(value, target_value)

                    action_loss = F.nll_loss(scaled_score.permute(0, 2, 1), action.long())
