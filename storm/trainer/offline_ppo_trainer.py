...
class OfflinePPOTrainer():
    ...
    def expert_environment(self,
        ...
        for step in range(0, self.config.num_steps):
            ...
            action = torch.tensor(info["expert_action"]).to(self.device)
            ...
            if "final_info" in info:
                for info_item in info["final_info"]:
                    if info_item is not None:
                        ...
                        self.writer.add_scalar(f"{prefix}/total_return", info_item["total_return"], self.global_step)
                        self.writer.add_scalar(f"{prefix}/total_profit", info_item["total_profit"], self.global_step)
        ...
        # Get values and logprobs
        with torch.no_grad():
            # Construct the input tensor
            input_tensor = TensorDict(
                {
                    "features": self.storage["features"],
                    "cashes": self.storage["policy_trading_cashes"],
                    "positions": self.storage["policy_trading_positions"],
                    "actions": self.storage["policy_trading_actions"],
                    "rets": self.storage["policy_trading_rets"],
                }, batch_size=self.storage.batch_size
            ).to(next_obs.device)
            input_action = self.storage["training_actions"].unsqueeze(-1).long()

            _, logprob, _, value = self.agent.get_action_and_value(input_tensor, input_action)

            self.storage["training_logprobs"] = logprob
            self.storage["training_values"] = value
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(self.config.num_steps)):
                ...
                self.storage["training_advantages"][t] = lastgaelam = delta + self.config.gamma * self.config.gae_lambda * nextnonterminal * lastgaelam
    ...
    def train(self):
        ...
        for update in range(1, num_updates + 1 + warm_up_updates):
            ...
            self.expert_environment(init_state=state, init_info=info, reset=False)
            ...
            self.writer.add_scalar(f"{prefix}/policy_learning_rate", self.policy_optimizer.param_groups[0]["lr"], self.global_step)
            self.writer.add_scalar(f"{prefix}/value_learning_rate", self.value_optimizer.param_groups[0]["lr"], self.global_step)
            self.writer.add_scalar(f"{prefix}/value_loss", trading_records["v_loss"].item(), self.global_step)
            self.writer.add_scalar(f"{prefix}/policy_loss", trading_records["pg_loss"].item(), self.global_step)
            self.writer.add_scalar(f"{prefix}/value", trading_records["value"].item(), self.global_step)
            self.writer.add_scalar(f"{prefix}/logprob", trading_records["logprob"].item(), self.global_step)
            self.writer.add_scalar(f"{prefix}/entropy", trading_records["entropy_loss"].item(), self.global_step)
            if self.use_bc:
                self.writer.add_scalar(f"{prefix}/bc_loss", trading_records["bc_loss"].item(), self.global_step)
            self.writer.add_scalar(f"{prefix}/old_approx_kl", trading_records["old_approx_kl"].item(), self.global_step)
            self.writer.add_scalar(f"{prefix}/approx_kl", trading_records["approx_kl"].item(), self.global_step)
            self.writer.add_scalar(f"{prefix}/total_approx_kl", trading_records["total_approx_kl"].item(), self.global_step)
            self.writer.add_scalar(f"{prefix}/policy_update_times", trading_records["policy_update_steps"] // self.config.gradient_checkpointing_steps, self.global_step)
            self.writer.add_scalar(f"{prefix}/clipfrac", num_clipfracs, self.global_step)
            self.writer.add_scalar(f"{prefix}/explained_variance", explained_var, self.global_step)
            self.writer.add_scalar(f"{prefix}/SPS", self.global_step / (time.time() - start_time), self.global_step)
    ...
    def valid(self, global_step):
            ...
            if "final_info" in info:
                for info_item in info["final_info"]:
                    if info_item is not None:
                        ...
                        self.writer.add_scalar(f"{prefix}/total_return", info_item["total_return"], global_step)
                        self.writer.add_scalar(f"{prefix}/total_profit", info_item["total_profit"], global_step)
        ...
        self.writer.add_scalar(f"{prefix}/ARR%", arr * 100, global_step)
        self.writer.add_scalar(f"{prefix}/SR", sr, global_step)
        self.writer.add_scalar(f"{prefix}/CR", cr, global_step)
        self.writer.add_scalar(f"{prefix}/SOR", sor, global_step)
        self.writer.add_scalar(f"{prefix}/DD", dd, global_step)
        self.writer.add_scalar(f"{prefix}/MDD%", mdd * 100, global_step)
        self.writer.add_scalar(f"{prefix}/VOL", vol, global_step)
        self.writer.add_scalar(f"{prefix}/TurnoverRate", turnover_rate, global_step)
        self.writer.add_scalar(f"{prefix}/NumTrades", num_trades, global_step)
        self.writer.add_scalar(f"{prefix}/NumBuys", num_buys, global_step)
        self.writer.add_scalar(f"{prefix}/NumSells", num_sells, global_step)
        self.writer.add_scalar(f"{prefix}/AvgHoldPeriod", avg_hold_period, global_step)
        self.writer.add_scalar(f"{prefix}/ActivityRate", activity_rate, global_step)
        self.writer.add_scalar(f"{prefix}/AvgTradeInterval", avg_trade_interval, global_step)
        self.writer.add_scalar(f"{prefix}/BuyToSellRatio", buy_to_sell_ratio, global_step)
